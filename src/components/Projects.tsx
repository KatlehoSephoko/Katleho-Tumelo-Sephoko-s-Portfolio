      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="rounded-2xl border border-zinc-800 bg-zinc-900/30 overflow-hidden flex flex-col lg:flex-row mb-16">
          <div className="p-8 lg:p-12 lg:w-1/2 flex flex-col justify-center">
            ...
            <div className="flex gap-4">
              <button className="px-4 py-2 bg-zinc-100 text-zinc-950 font-medium rounded hover:bg-white transition-colors">
                View Project
              </button>
              <a href={featuredProject.github} className="px-4 py-2 border border-zinc-700 text-zinc-300 rounded hover:bg-zinc-800 transition-colors flex items-center gap-2">
        ...
        <div className="mb-8 flex flex-wrap gap-2">
          {categories.map(cat => (
            <button 
              ...
              className={`px-3 py-1.5 text-xs rounded border transition-colors ${filter === cat ? "bg-emerald-500/10 border-emerald-500/50 text-emerald-400" : "bg-zinc-950 border-zinc-800 text-zinc-400 hover:border-zinc-600"}`}
            >
              {cat}
            </button>
          ))}
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredProjects.map(project => (
            <div key={project.title} className="p-6 rounded-xl border border-zinc-800 bg-zinc-900/20 hover:border-zinc-700 transition-colors flex flex-col h-full group">
              ...
              <div className="flex justify-between items-center pt-4 border-t border-zinc-800/50">
                <button onClick={() => setSelectedProject(project)} className="text-sm text-zinc-300 hover:text-white flex items-center gap-1 group-hover:text-emerald-400 transition-colors">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-zinc-950/80 backdrop-blur-sm">
          <div className="bg-zinc-900 border border-zinc-800 rounded-xl w-full max-w-2xl max-h-[90vh] overflow-y-auto shadow-2xl flex flex-col">
            <div className="sticky top-0 bg-zinc-900/95 backdrop-blur-sm p-6 border-b border-zinc-800 flex justify-between items-start">
              ...
              <button onClick={() => setSelectedProject(null)} className="p-2 text-zinc-400 hover:text-white bg-zinc-800/50 hover:bg-zinc-800 rounded-full transition-colors">
                <X size={20} />
              </button>
            </div>
            ...
            <div className="p-6 border-t border-zinc-800 bg-zinc-950/50 flex gap-4 mt-auto">
              <button className="px-4 py-2 border border-zinc-700 text-zinc-300 rounded hover:bg-zinc-800 transition-colors flex items-center gap-2 text-sm">
